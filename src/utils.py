...
class Retriever:
    ...
    def __init__(self, ctx, retriever_name="ncbi/MedCPT-Query-Encoder", corpus_name="textbooks", db_dir="./corpus", HNSW=False, **kwarg):
        self.retriever_name = retriever_name
        self.corpus_name = corpus_name
        self.ctx = ctx
        self.timing = []

        self.doc_serve_path = ctx['doc_serve_path']
        with open(self.doc_serve_path, 'r') as f:
            self.doc_serve = json.load(f)
        self.doc_serve_amt = 0
    ...
    def get_relevant_documents(self, question, k=32, id_only=False, proxcache = None, stats = None, tolerance = -1.0, **kwarg):
        assert type(question) == str
        k_db, k_ret = k['db'], k['ret']
        if type(tolerance) == str:
            tolerance = float(tolerance[4:])
            autotol = True
        else:
            autotol = False
        ...
        if "bm25" in self.retriever_name.lower():
            res_ = [[]]
            hits = self.index.search(question[0], k=k_db)
            ...
            to_search = query_embed[0]

            # look in cache
            t1 = time.time()
            cache_res = proxcache.find(list(to_search))

            if cache_res is None:
                # print("no hit")
                res_ = self.index.search(query_embed, k=k_db)
                indices = res_[1][0]
                scores = res_[0][0]
                if autotol:
                    proxcache.insert(list(to_search), (list(indices), list(scores)), np.min(scores) / tolerance)
                else:
                    proxcache.insert(list(to_search), (list(indices), list(scores)), tolerance)

            else:
                # print("hit")
                indices, scores = cache_res
                scores = np.array([scores])
                indices = np.array([indices])
                res_  = (scores, indices)
                stats["hit_amt"] += 1

            # print('performing reraknking')
            closest_vectors = self.index.reconstruct_batch(res_[1][0])
            closest_indices = np.argsort(np.sum((closest_vectors - query_embed) ** 2, axis=1))[:k_ret]

            x = [res_[0][0][closest_indices]]
            y = [res_[1][0][closest_indices]]

            res_ = (np.array(x), y)
            # print('done reraknking')
            ...
            t2 = time.time()
            stats["time_fetch_sum"] += (t2 - t1)
            #print(indices)
        scores = res_[0][0].tolist()
        stats["norm"].append(np.mean(scores))
        if id_only:
...
class RetrievalSystem:
    ...
    def __init__(self, ctx, retriever_name="MedCPT", corpus_name="Textbooks", db_dir="./corpus", HNSW=False, cache=False):
        self.ctx = ctx
        self.retriever_name = retriever_name
        self.corpus_name = corpus_name
        ...
        for retriever in retriever_names[self.retriever_name]:
            ...
            for corpus in corpus_names[self.corpus_name]:
                self.retrievers[-1].append(Retriever(ctx, retriever, corpus, db_dir, HNSW=HNSW))
        self.cache = cache
        if self.cache:
    ...
    def retrieve(self, question, k=32, rrf_k=100, id_only=False, proxcache = None, stats = None, tolerance=-1.0):
        '''
        ...
        for i in range(len(retriever_names[self.retriever_name])):
            ...
            for j in range(len(corpus_names[self.corpus_name])):
                t, s = self.retrievers[i][j].get_relevant_documents(question, k=k_, id_only=id_only, proxcache = proxcache, stats = stats, tolerance=tolerance)
                texts[-1].append(t)
                scores[-1].append(s)
        texts, scores = self.merge(texts, scores, k=k['ret'], rrf_k=rrf_k)
        if self.cache:
