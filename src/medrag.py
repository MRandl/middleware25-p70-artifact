...
class MedRAG:
    ...
    def __init__(self, ctx, llm_name="OpenAI/gpt-3.5-turbo-16k", rag=True, follow_up=False, retriever_name="MedCPT", corpus_name="Textbooks", db_dir="./corpus", cache_dir=None, corpus_cache=False, HNSW=False):
        self.ctx = ctx
        ...
        self.docExt = None
        if rag:
            self.retrieval_system = RetrievalSystem(self.ctx, self.retriever_name, self.corpus_name, self.db_dir, cache=corpus_cache, HNSW=HNSW)
        else:
    ...
    def medrag_answer(self, question, options=None, k=32, rrf_k=100, tolerance = -1.0, save_dir = None, snippets=None, snippets_ids=None, proxcache = None, stats = None, acc_cache = None, **kwargs):
        '''
        ...
        if self.rag:
            if snippets is not None:
                ...
                retrieved_snippets, scores = self.retrieval_system.retrieve(question, k=k, rrf_k=rrf_k, proxcache = proxcache, stats = stats, tolerance = tolerance)
        ...
        if not self.rag:
            ...
            for context in contexts:
                ...
                # print('looking in acc_cache')
                if prompt_medrag not in acc_cache: # we dont care about e2e latency, so might as well cache this to save on gpu time
                    # print('acc not hit')
                    ans = self.generate(messages, **kwargs)
                    acc_cache[prompt_medrag] = ans
                else:
                    # print('acc hit')
                    ans = acc_cache[prompt_medrag]
